...
async def invite(ctx: commands.Context, member : Member):
    ...
    async def yesButton_callback ():
        ...
        sent_msg.edit("Invite Accepted!")
    async def noButton_callback():
        await InteractionResponse.send_message ("Invite Declined")

    yesButton.callback = yesButton_callback
    noButton.callback = noButton_callback

    if (leaguebase.checkOwnership(disc_id)):
        sent_msg = await ctx.send (member.mention + "Do you want to accept the invite from League " + leaguebase.getOwnedLeague(disc_id) + "?", view=view)
    else:
